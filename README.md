...
...what the above command does...is add the selected user name to become a 'sudo'/or, 'super user do' on the Linux machine;    
which means, whenever they wish to use priveliged commands they just need to prefix that command with the term: sudo;      

>> sudo apt-get install softwarePackageName  
...
>>apt install ncal        <!--installs the calender application  
>>cal                     <--(shows calender/current month)    
